...
class FieldManager(object):
    def __init__(self, dac):
        ...
        self.VsupplyX = 24.11 #[V]
        self.VsupplyY = 24.15 #[V]
        self.VsupplyZ = 24.09 #[V]
    ...
    def setX(self, mT):
        ...
        # supply voltage.
        if mT>=0:
            self.dac.output_voltage_pwm(self.pinXpwm, self.pinXdir, 0.0044*(mT**3)-0.0368*(mT**2)+1.4143*mT, self.VsupplyX)
        else:
            self.dac.output_voltage_pwm(self.pinXpwm, self.pinXdir, 0.0044*(mT**3)+0.0368*(mT**2)+1.4143*mT, self.VsupplyX)
        self.bxSetpoint = mT
    ...
    def setY(self, mT):
        ...
        # supply voltage.
        if mT>=0:
            self.dac.output_voltage_pwm(self.pinYpwm, self.pinYdir, 0.0028*(mT**3)-0.0327*(mT**2)+0.9149*mT, self.VsupplyY)
        else:
            self.dac.output_voltage_pwm(self.pinYpwm, self.pinYdir, 0.0028*(mT**3)+0.0327*(mT**2)+0.9149*mT, self.VsupplyY)
        self.bySetpoint = mT
    ...
    def setZ(self, mT):
        ...
        # supply voltage.
        if mT>=0:
            self.dac.output_voltage_pwm(self.pinZpwm, self.pinZdir, 0.0009*(mT**3)-0.0123*(mT**2)+0.5381*mT, self.VsupplyZ)
        else:
            self.dac.output_voltage_pwm(self.pinZpwm, self.pinZdir, 0.0009*(mT**3)+0.0123*(mT**2)+0.5381*mT, self.VsupplyZ)
        self.bzSetpoint = mT
