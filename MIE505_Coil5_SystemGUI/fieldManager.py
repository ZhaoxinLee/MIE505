...
class FieldManager(object):
    ...
    def setX(self, mT):
        ...
        # supply voltage.
        if mT>=0:
            self.dac.output_voltage_pwm(self.pinXpwm, self.pinXdir, 0.0052*(mT**3)-0.0482*(mT**2)+1.458*mT, self.VsupplyX)
        else:
            self.dac.output_voltage_pwm(self.pinXpwm, self.pinXdir, 0.0052*(mT**3)+0.0482*(mT**2)+1.458*mT, self.VsupplyX)
        self.bxSetpoint = mT
    ...
    def setY(self, mT):
        ...
        # supply voltage.
        if mT>=0:
            self.dac.output_voltage_pwm(self.pinYpwm, self.pinYdir, 0.0041*(mT**3)-0.0479*(mT**2)+0.974*mT, self.VsupplyY)
        else:
            self.dac.output_voltage_pwm(self.pinYpwm, self.pinYdir, 0.0041*(mT**3)+0.0479*(mT**2)+0.974*mT, self.VsupplyY)
        self.bySetpoint = mT
    ...
    def setZ(self, mT):
        ...
        # supply voltage.
        if mT>=0:
            self.dac.output_voltage_pwm(self.pinZpwm, self.pinZdir, 0.0008*(mT**3)-0.0098*(mT**2)+0.5245*mT, self.VsupplyZ)
        else:
            self.dac.output_voltage_pwm(self.pinZpwm, self.pinZdir, 0.0008*(mT**3)+0.0098*(mT**2)+0.5245*mT, self.VsupplyZ)
        self.bzSetpoint = mT
