...
class FieldManager(object):
    def __init__(self, dac):
        ...
        self.VsupplyX = 24.15 #[V]
        self.VsupplyY = 24.14 #[V]
        self.VsupplyZ = 23.93 #[V]
    ...
    def setX(self, mT):
        ...
        # supply voltage.
        if mT>=0:
            self.dac.output_voltage_pwm(self.pinXpwm, self.pinXdir, 0.0053*(mT**3)-0.0418*(mT**2)+1.4278*mT , self.VsupplyX)
        else:
            self.dac.output_voltage_pwm(self.pinXpwm, self.pinXdir, 0.0053*(mT**3)+0.0418*(mT**2)+1.4278*mT , self.VsupplyX)
        self.bxSetpoint = mT
    ...
    def setY(self, mT):
        ...
        # supply voltage.
        if mT>=0:
            self.dac.output_voltage_pwm(self.pinYpwm, self.pinYdir, 0.0042*(mT**3)-0.0449*(mT**2)+0.9613*mT , self.VsupplyY)
        else:
            self.dac.output_voltage_pwm(self.pinYpwm, self.pinYdir, 0.0042*(mT**3)+0.0449*(mT**2)+0.9613*mT , self.VsupplyY)
        self.bySetpoint = mT
    ...
    def setZ(self, mT):
        ...
        # supply voltage.
        if mT>=0:
            self.dac.output_voltage_pwm(self.pinZpwm, self.pinZdir, 0.0012*(mT**3)-0.0161*(mT**2)+0.554*mT, self.VsupplyZ)
        else:
            self.dac.output_voltage_pwm(self.pinZpwm, self.pinZdir, 0.0012*(mT**3)+0.0161*(mT**2)+0.554*mT , self.VsupplyZ)
        self.bzSetpoint = mT
