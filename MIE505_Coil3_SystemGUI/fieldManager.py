...
class FieldManager(object):
    def __init__(self, dac):
        ...
        self.VsupplyX = 24.05 #[V]
        self.VsupplyY = 24.08 #[V]
    ...
    def setX(self, mT):
        ...
        # supply voltage.
        if mT>=0:
            self.dac.output_voltage_pwm(self.pinXpwm, self.pinXdir, 0.0051*(mT**3)-0.0439*(mT**2)+1.4407*mT  , self.VsupplyX)
        else:
            self.dac.output_voltage_pwm(self.pinXpwm, self.pinXdir, 0.0051*(mT**3)+0.0439*(mT**2)+1.4407*mT, self.VsupplyX)
        self.bxSetpoint = mT
    ...
    def setY(self, mT):
        ...
        # supply voltage.
        if mT>=0:
            self.dac.output_voltage_pwm(self.pinYpwm, self.pinYdir, 0.0033*(mT**3)-0.0366*(mT**2)+0.9412*mT, self.VsupplyY)
        else:
            self.dac.output_voltage_pwm(self.pinYpwm, self.pinYdir, 0.0033*(mT**3)+0.0366*(mT**2)+0.9412*mT, self.VsupplyY)
        self.bySetpoint = mT
    ...
    def setZ(self, mT):
        ...
        # supply voltage.
        if mT>=0:
            self.dac.output_voltage_pwm(self.pinZpwm, self.pinZdir, 0.0008*(mT**3)-0.0104*(mT**2)+0.5237*mT, self.VsupplyZ)
        else:
            self.dac.output_voltage_pwm(self.pinZpwm, self.pinZdir, 0.0008*(mT**3)+0.0104*(mT**2)+0.5237*mT, self.VsupplyZ)
        self.bzSetpoint = mT
