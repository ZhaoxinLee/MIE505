...
class Vision(object):
    def __init__(self, resolution= (640,480), framerate=30, exposure_mode='sports'): #    (320,240)(480,320)(640,480)(1024,768), (2028,1520)
        #self._id = index
        #self._type = type
        #self._guid = guid
    ...
    def infiniteFrameProcess(self): #this runs in its own thread. Acquire images and perform filtering/detection
        ...
        for f in self.stream:
            ...
            # preparation for the next >>> Step to point {} <<<frame
            ...
            if self.isSnapshotEnabled():
                ...
                cv2.imwrite(os.getcwd()+'/snapshot/snapshot_'+timestr+'.png',filterlib.color(self.frameProcessed))
                self.setStateSnapshotEnabled(False)
            if self.videoWritingState:
                self.videoWriter.write(filterlib.color(self.frameProcessed))
    ...
    def startRecording(self):
        timestr = time.strftime("%Y%m%d_%H%M%S")
        self.videoWriter = cv2.VideoWriter(os.getcwd()+'/video/'+timestr+'.avi',cv2.VideoWriter_fourcc('M','J','P','G'),22, (640,480))
